...
@section('contents')
        ...
        <div class="card border-0 shadow-sm rounded">
            <div class="card-header bg-white border-0 d-flex justify-content-between align-items-center py-3">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <h6 class="mb-0 fw-bold">Edit MBBS Course Category with Subcategories</h6>
                </div>
            ...
            <div class="card-body">
                ...
                <form action="{{ route('mbbs-course.update', $category->id) }}" method="POST" class="needs-validation" novalidate>
                    @csrf
                    @method('PUT')
                            ...
                            <div class="row">
                                <div class="col-md-12 mb-3">
                                    ...
                                    <input type="text" class="form-control @error('parent_name') is-invalid @enderror" 
                                           id="parent_name" name="parent_name" value="{{ old('parent_name', $category->name) }}" required>
                                    @error('parent_name')
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea class="form-control @error('parent_description') is-invalid @enderror" 
                                              id="parent_description" name="parent_description" rows="3">{{ old('parent_description', $category->description) }}</textarea>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" class="form-control @error('parent_order') is-invalid @enderror" 
                                           id="parent_order" name="parent_order" value="{{ old('parent_order', $category->order) }}" min="0">
                                    @error('parent_order')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div class="form-check form-switch mt-2">
                                        <input class="form-check-input" type="checkbox" id="parent_is_active" 
                                               name="parent_is_active" {{ old('parent_is_active', $category->is_active) ? 'checked' : '' }}>
                    ...
                    <div class="card border-info">
                        ...
                        <div class="card-body">
                            <div id="childCategoriesContainer">
                                @php
                                    $childCategoriesList = $childCategories ?? collect();
                                    $hasChildren = $childCategoriesList->count() > 0;
                                @endphp

                                @if($hasChildren)
                                    @foreach($childCategoriesList as $childIndex => $childCategory)
                                        <div class="child-row mb-3 p-3 border rounded" data-child-id="{{ $childCategory->id }}">
                                            <input type="hidden" name="child_ids[]" value="{{ $childCategory->id }}">
                                            <div class="row g-2">
                                                <div class="col-md-5">
                                                    <label class="form-label">Child Category Name</label>
                                                    <input type="text" class="form-control child-name" 
                                                           name="child_names[]" 
                                                           value="{{ old('child_names.' . $childIndex, $childCategory->name) }}"
                                                           placeholder="e.g., Department of Anatomy">
                                                </div>
                                                <div class="col-md-5">
                                                    <label class="form-label">Description (Optional)</label>
                                                    <input type="text" class="form-control child-description" 
                                                           name="child_descriptions[]" 
                                                           value="{{ old('child_descriptions.' . $childIndex, $childCategory->description) }}"
                                                ...
                                                <div class="col-md-2">
                                                    ...
                                                    <input type="number" class="form-control child-order" 
                                                           name="child_orders[]" 
                                                           value="{{ old('child_orders.' . $childIndex, $childCategory->order) }}"
                                                           min="0">
                                                </div>
                                                <div class="col-md-12">
                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input child-active" type="checkbox" 
                                                               ...
                                                               value="1"
                                                               {{ old('child_is_active.' . $childIndex, $childCategory->is_active) ? 'checked' : '' }}>
                                                        <label class="form-check-label">Active</label>
                                                    </div>
                                                    <div class="mt-2">
                                                        <input type="hidden" name="delete_children[]" value="" class="delete-child-flag">
                                    ...
                                    @endforeach
                                @else
                                    <!-- Default empty row if no children exist -->
                                    <div class="child-row mb-3 p-3 border rounded">
                                        <input type="hidden" name="child_ids[]" value="">
                                        <div class="row g-2">
                                            <div class="col-md-5">
                                                <label class="form-label">Child Category Name</label>
                                                <input type="text" class="form-control child-name" 
                            ...
                            <div class="text-muted small mt-2">
                                <i class="fas fa-info-circle"></i> Leave child category name empty if you don't want to add that subcategory. Click "Remove" to delete existing child categories.
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-end gap-2 mt-4">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save"></i> Update Parent & Child Categories
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        let rowCounter = {{ $hasChildren ? $childCategoriesList->count() + 1 : 2 }};
        document.getElementById('addChildRow').addEventListener('click', function() {
            ...
            newRow.querySelector('.remove-row').addEventListener('click', function() {
                handleRowRemove(newRow);
            });
        });

        // Remove row functionality
        function handleRowRemove(rowElement) {
            const childId = rowElement.getAttribute('data-child-id');
            const deleteFlag = rowElement.querySelector('.delete-child-flag');
            
            if (childId) {
                // Existing child category - mark for deletion
                if (deleteFlag) {
                    deleteFlag.value = childId;
                }
                rowElement.style.display = 'none'; // Hide instead of remove to preserve form data
            } else {
                // New row - just remove it
                const allRows = document.querySelectorAll('.child-row:not([style*="display: none"])');
                if (allRows.length > 1) {
                    rowElement.remove();
                } else {
